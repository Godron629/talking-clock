...
from talking_clock import talking_clock

class TestTalkingClock(TestCase):
	def test_all_zeros(self):
		self.assertEqual(talking_clock("00:00"), "It's twelve am")
	...
	def test_single_digit_minutes(self):
		...
		for i in test_cases: 
			result = talking_clock("00:0{}".format(i[0]))
	...
	def test_tens_plus_single_digits(self):
		...
		for i in test_cases: 
			result = talking_clock("00:2{}".format(i[0]))
	...
	def test_all_minute_tens(self):
		...
		for i in test_cases:
			result = talking_clock("00:{}".format(i[0]))
			expected_result = "It's twelve {} am".format(i[1])
			self.assertEqual(result, expected_result)

	def test_over_sixty_minutes(self):
		self.assertEqual(talking_clock("00:65"), "It's twelve oh five am")

	def test_over_twenty_four_hours(self):
		self.assertEqual(talking_clock("26:00"), "It's two am")

	def test_negative_hours(self):
		self.assertEqual(talking_clock("-2:00"), "It's ten pm")

	def test_negative_minutes(self):
		self.assertEqual(talking_clock("00:-10"), "It's twelve fifty am")

	def test_not_a_time(self):
		self.assertRaisesRegexp(ValueError, "Time not in format",
			talking_clock, "blaaa, cheese 23984742")
	...
	def test_reddit_sample_data(self):
		...
		for i in test_cases:
			self.assertEqual(talking_clock(i[0]), i[1])
